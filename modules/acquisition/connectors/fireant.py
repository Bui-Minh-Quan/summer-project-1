...
class FireAntConnector(BaseConnector):
    ...
    def map_document(self, raw: RawDocument) -> Document | None:
        try:
            ...
            symbols = [
                s["symbol"] for s in tagged if isinstance(s, dict) and s.get("symbol")
            ]
            ...
            return Document(
                ...
                author=payload.get("creator", {}).get("name")
    ...
    def _crawl_feed(
        ...
        # Normalize string inputs to timezone-aware datetime objects
        ...
        start_date_dt = _to_dt(start_date)
        end_date_dt = _to_dt(end_date)
        watermark_dt = _to_dt(watermark)
        ...
        if end_date_dt:
            offset = self._find_target_offset(api_type, end_date_dt, batch_size)
        else:
            offset = 0
        ...
        while not terminated and len(collected_docs) < limit:
            ...
            if not batch:
                logger.info("Empty batch received or API error. Stopping crawl.")
                break
            ...
            for raw_item in batch_items:
                ...
                if watermark_dt and pub_date <= watermark_dt:
                    logger.info(
                        f"Reached watermark ({pub_date} <= {watermark_dt}). Terminating."
                    )
                    terminated = True
                    break

                # 2. Check upper bound for historical fetches
                if end_date_dt and pub_date > end_date_dt:
                    # Item is too new, skip and keep paginating backward
                    continue

                # 3. Check lower bound (For historical fetches)
                if start_date_dt and pub_date < start_date_dt:
                    logger.info(
                        f"Reached historical lower bound ({pub_date} < {start_date_dt}). Terminating."
                    )
                    terminated = True
                    break
        ...
        logger.info(
            f"Crawl completed for {doc_type.value}. Total fetched: {len(collected_docs)}"
        )
    ...
    def _fetch_news_details(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_id = {
                executor.submit(_fetch_single, post_id): post_id
    ...
    def _probe_date(self, api_type: int, offset: int) -> datetime | None:
        ...
        res = self._get(
            self.BASE_URL, params={"type": api_type, "offset": offset, "limit": 1}
        )
    ...
    def _find_target_offset(
        self, api_type: int, target_date: datetime, batch_size: int
    ) -> int:
        # Uses Exponential search followed by binary search to locate starting offset
        # for a specific historical target_date
        logger.info(
            f"Probing API to find starting offset for date: {target_date.strftime('%Y-%m-%d')}..."
        )
        ...
        while True:
            ...
            high *= 2
            logger.debug(
                f"Exponential leap: offset {high} (Date: {date_high.strftime('%Y-%m-%d')})"
            )
        ...
        logger.info(
            f"Found target window around offset {result_offset}. Starting crawl safely at offset {safe_start_offset}."
        )
