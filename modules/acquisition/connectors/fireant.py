...
class FireAntConnector(BaseConnector):
    ...
    def _map_document(self, raw: Dict[str, Any], doc_type: DocumentType) -> Optional[Document]:
        try: 
            doc_id = str(raw.get("postID") or raw.get("id"))
            if not doc_id:
                return None 
        
            tagged = raw.get("taggedSymbols", [])
            symbols = [
            ...
            if doc_type == DocumentType.NEWS:
                ...
                content = raw.get("originalContent") or raw.get("content")
            
            # Parse timestamp 
            pub_date = self._parse_date(raw.get("date"))

            # Isolate metadate 
            primary_keys = {"postID", "id", "title", "content", "originalContent", 
                            "description", "date", "taggedSymbols"}
            metadata = {k: v for k, v in raw.items() if k not in primary_keys}

            # Enrich metadata with domain-specific metrics
            metadata["sentiment_score"] = raw.get("sentiment")
            metadata['likes'] = raw.get("totalLikes", 0)
            metadata["shares"] = raw.get("totalShares", 0)
            metadata["replies"] = raw.get("totalReplies", 0)
            ...
            return Document(
                ...
                title=title,
                content=content,
                raw_html=raw.get("content") if doc_type == DocumentType.NEWS else None,
                author=raw.get("creator", {}).get("name") if isinstance(raw.get("creator"), dict) else None,
            ...
            logger.warning(f"Failed to map document {raw.get('postID')}: {e}")
            return None
...
if __name__ == "__main__":
    load_dotenv() 
    bearer_token = os.getenv("Fire_Ant_Bearer")

    test = FireAntConnector(bearer_token=bearer_token)
    health = test.health_check()

    if health:
        print("Connector is ok")

    # 1. Test News Ingestion
    news = test.fetch_latest_news(limit=10)
    # Check if 'news' is a list, has items, and the first item is a Document
    print(len(news))
    if news and isinstance(news, list) and isinstance(news[0], Document):
        #print(f"Latest News Content: {news[9]}")  # Dot notation!
        print(news[0])

    # 2. Test Posts Ingestion
    posts = test.fetch_latest_posts(limit=10)
    print(len(posts))
    if posts and isinstance(posts, list) and isinstance(posts[0], Document):
        pass
        # print(f"Latest Post Content: {posts[3]}")  # Dot notation!
    
    # 3. Test Historical Ingestion
