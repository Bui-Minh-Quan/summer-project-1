            ...
            if isinstance(quote.timestamp, str):
                try:
                    ...
                    parsed_dt = datetime.fromisoformat(
                        quote.timestamp.replace(" ", "T")
                    )
                    quote.timestamp = parsed_dt.replace(tzinfo=timezone.utc)
                except ValueError:
                    logger.warning(
                        f"Dropping quote for {quote.ticker}: Unparseable timestamp {quote.timestamp}"
                    )
                    continue
            elif (
                isinstance(quote.timestamp, datetime) and quote.timestamp.tzinfo is None
            ):
...
class MarketValidator:
    ...
    def validate(cls, quote: RawMarketQuote) -> tuple[bool, list[str]]:
        ...
        if quote.high_price < quote.low_price:
            errors.append(
                f"Math violation: High ({quote.high_price}) < Low ({quote.low_price})."
            )
        ...
        if isinstance(quote.timestamp, datetime) and quote.timestamp > datetime.now(
            timezone.utc
        ):
...
class MarketDeduplicator:
    ...
    def process_and_deduplicate(
        cls, valid_quotes: list[RawMarketQuote]
    ) -> list[MarketQuote]:
        ...
        for raw in valid_quotes:
            assert isinstance(raw.timestamp, datetime), (
                "Timestamp must be datetime by deduplication stage"
            )
