...
class Document(BaseModel):
    ...
    fingerprint: str | None = Field(
        default=None, description="SHA-256 deterministic content hash for deduplication"
    )
    ...
    raw_html: str | None = Field(
        default=None, description="Original HTML before cleaning"
    )
    ...
    symbols: list[str] = Field(
        default_factory=list, description="Mentioned stock symbol"
    )
    ...
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Connector-specific metadata"
    )
